...
//            println()
    ...
    fun rungeKutta(x: DoubleArray, y:DoubleArray, n: Int, h: Double): DoubleArray {
        for(i in 0..y.size-2) {
            val k0 = f(x[i], y[i]);
    ...
    fun adams(x: DoubleArray, y:DoubleArray, n: Int, h: Double): DoubleArray {
        val f: DoubleArray = DoubleArray(x.size)
        f[0] = f(x[0],y[0])
        x[1] = x[0]+h
        y[1] = y[0]+h*f[0]
        f[1] = f(x[1],y[1])
